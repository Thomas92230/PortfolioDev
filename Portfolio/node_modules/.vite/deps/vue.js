...
function trigger$1(target, type, key, newValue, oldValue, oldTarget) {
	...
	else if (key === "length" && isArray(target)) depsMap.forEach((dep, key) => {
...
function triggerEffects(dep, debuggerEventExtraInfo) {
	for (const effect of isArray(dep) ? dep : [...dep]) if (effect !== activeEffect || effect.allowRecurse) {
...
function createSetter(shallow = false) {
	return function set(target, key, value, receiver) {
		...
		if (target === toRaw(receiver)) {
			if (!hadKey) trigger$1(target, "add", key, value);
			else if (hasChanged(value, oldValue)) trigger$1(target, "set", key, value, oldValue);
		}
		return result;
	};
}
function deleteProperty(target, key) {
	...
	if (result && hadKey) trigger$1(target, "delete", key, void 0, oldValue);
	return result;
}
...
function deleteEntry(key) {
	...
	if (hadKey) trigger$1(target, "delete", key, void 0, oldValue);
	return result;
}
function clear() {
	const target = toRaw(this);
	...
	if (hadItems) trigger$1(target, "clear", void 0, void 0, oldTarget);
	return result;
}
...
function emit$1(instance, event, ...rawArgs) {
	...
	if (modelArg && modelArg in props) {
		...
		else if (number) args = rawArgs.map(toNumber);
	}
...
function matches(pattern, name) {
	...
	else if (isString(pattern)) return pattern.split(",").indexOf(name) > -1;
...
function updateProps(instance, rawProps, rawPrevProps, optimized) {
	...
	if (hasAttrsChanged) trigger$1(instance, "set", "$attrs");
	validateProps(rawProps || {}, props, instance);
}
		...
		if (_isString || _isRef) {
			const doSet = () => {
				if (rawRef.f) {
					...
					else if (!isArray(existing)) if (_isString) refs[ref] = [refValue];
					else {
						ref.value = [refValue];
...
function isSameVNodeType(n1, n2) {
	if (n2.shapeFlag & 6 && hmrDirtyComponents.has(n2.type)) return false;
...
function mergeProps(...args) {
	...
	for (let i = 0; i < args.length; i++) {
		...
		for (const key in toMerge) if (key === "class") {
			...
			if (existing !== incoming && !(isArray(existing) && existing.includes(incoming))) ret[key] = existing ? [].concat(existing, incoming) : incoming;
...
function createSlots(slots, dynamicSlots) {
	for (let i = 0; i < dynamicSlots.length; i++) {
		...
		else if (slot) slots[slot.name] = slot.fn;
	}
	return slots;
}
/**
...
function renderSlot(slots, name, props = {}, fallback, noSlotted) {
	if (currentRenderingInstance.isCE) return createVNode("slot", name === "default" ? null : { name }, fallback && fallback());
	let slot = slots[name];
...
var PublicInstanceProxyHandlers = {
	get({ _: instance }, key) {
		...
		if (publicGetter) {
			...
			if (data !== EMPTY_OBJ && (key[0] === "$" || key[0] === "_") && hasOwn(data, key)) warn(`Property ${JSON.stringify(key)} must be accessed via $data because it starts with a reserved character ("$" or "_") and is not proxied on the render context.`);
	...
	set({ _: instance }, key, value) {
		...
		if (setupState !== EMPTY_OBJ && hasOwn(setupState, key)) setupState[key] = value;
...
function getExposeProxy(instance) {
	if (instance.exposed) return instance.exposeProxy || (instance.exposeProxy = new Proxy(proxyRefs(markRaw(instance.exposed)), { get(target, key) {
...
function isMemoSame(cached, memo) {
	...
	for (let i = 0; i < prev.length; i++) if (prev[i] !== memo[i]) return false;
...
function setStyle(style, name, val) {
	...
	else if (name.startsWith("--")) style.setProperty(name, val);
	else {
...
function createInvoker(initialValue, instance) {
	const invoker = (e) => {
		...
		if (skipTimestampCheck || timeStamp >= invoker.attached - 1) callWithAsyncErrorHandling(patchStopImmediatePropagation(e, invoker.value), instance, 5, [e]);
	};
	...
	*/
	_resolveDef() {
		...
		const resolve = (def) => {
			...
			for (const key of Object.keys(this)) if (key[0] !== "_") this._setProp(key, this[key], true, false);
			for (const key of rawKeys.map(camelize)) Object.defineProperty(this, key, {
				get() {
	...
	_setAttr(key) {
		...
		if (this._numberProps && this._numberProps[key]) value = toNumber(value);
...
function validateDuration(val) {
	if (typeof val !== "number") warn(`<transition> explicit duration is not a valid number - got ${JSON.stringify(val)}.`);
	else if (isNaN(val)) warn("<transition> explicit duration is NaN - the duration expression might be incorrect.");
}
...
function callModelHook(el, binding, vnode, prevVNode, hook) {
	...
	switch (el.tagName) {
		...
		case "TEXTAREA":
			...
			break;
		default: switch (vnode.props && vnode.props.type) {
			case "checkbox":
